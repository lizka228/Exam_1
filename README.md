...
Сохранение результатов (save_results)
  ...
  3) Запись в существующий файл в Яндекс.Диск → поскольку оркестрация выполняется 1 раз в день, то файл записывается с датой. Если выполняется несколько раз за день (Запуск вручную), то идёт перезапись файла текущей даты.
